	...
	'artwork_movement': [
		{
			...
			templates: [
				"{{ title }} appartient {{#ifCond movements.length '>' 1}}aux courants artistiques {{else}} au courant artistique {{/ifCond}} {{#movements}}{{#ifCond @last '==' true }} {{else}}, {{/ifCond}}{{{ fields.name }}}{{/movements}}"
			]
		}
	],
}
