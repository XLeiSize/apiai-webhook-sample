	...
	'artwork_movement': [
		{
			...
			templates: [
				"{{ title }} appartient {{#ifCond movements.length '>' 1}}aux courants artistiques {{else}} au courant artistique {{/ifCond}} {{#movements}} {{{ fields.name }}} {{#math @index '+' 1}}/{{ length }}  {{#ifCond length '>' #math @index '+' 1 }}{{#ifCond length '==' 2}} et {{else}}, {{/ifCond}}{{/ifCond}}{{/movements}}"
			]
		}
	],
}
