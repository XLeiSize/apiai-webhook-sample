	...
	'artwork_movement': [
		{
			...
			templates: [
				"{{ title }} appartient {{#ifCond movements.length '>' 1}}aux courants artistiques {{else}} au courant artistique {{/ifCond}} {{#each movements}} {{{ this.fields.name }}} {{#math @index '+' 1}}/{{ this.length }} {{/each}}"
			]
		}
	],
}
